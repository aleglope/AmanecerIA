...
import { NotificationPreferences, NotificationTone, NotificationLength } from '../types';
import { NOTIFICATION_TONES, NOTIFICATION_LENGTHS } from '../constants';
import { subscribeUserToPush, unsubscribeUserFromPush } from '../utils/notificationUtils';
...
const SettingsPage: React.FC<SettingsPageProps> = ({ onBack }) => {
    ...
    const { user, updateUserName, updateUserPushSubscription, updateUserNotificationPreferences } = useContext(AuthContext);

    const [name, setName] = useState(user?.name || '');
    const [avatarError, setAvatarError] = useState('');
    const [prefs, setPrefs] = useState<NotificationPreferences>(user?.notificationPreferences || { tone: 'Amable', length: 'Medio' });
    ...
    useEffect(() => {
        ...
        if (user) {
            setName(user.name || '');
            setPrefs(user.notificationPreferences || { tone: 'Amable', length: 'Medio' });
        }
    ...
    const handleSubscriptionToggle = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const isPermissionError = err.name === 'NotAllowedError' || (err.message && err.message.toLowerCase().includes('permission denied'));
    ...
    const handlePrefChange = (field: keyof NotificationPreferences, value: NotificationTone | NotificationLength) => {
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-night-blue">
            ...
            <main className="container mx-auto px-4 sm:px-6 py-12">
                ...
                <div className="text-center mb-12">
                    <h1 className="text-4xl md:text-5xl font-bold text-gray-800 dark:text-night-text">{t('settingsPage.title')}</h1>
                </div>

                <div className="max-w-2xl mx-auto space-y-12">
                    {/* Profile Section */}
                    <section>
                        <h2 className="text-2xl font-semibold border-b border-gray-200 dark:border-gray-700 pb-2 mb-6">{t('settingsPage.profileSection.title')}</h2>
                        <div className="space-y-6">
                            ...
                            <div>
                                <label htmlFor="name" className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('auth.nameLabel')}</label>
                                <input
                                    type="text"
                                    id="name"
                                    value={name}
                    ...
                    <section>
                        <h2 className="text-2xl font-semibold border-b border-gray-200 dark:border-gray-700 pb-2 mb-6">{t('settingsPage.notificationsSection.title')}</h2>
                        <div className="space-y-6">
                            <div className="flex items-center justify-between bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                                <div>
                                    <h3 className="font-semibold text-gray-800 dark:text-night-text">{t('settingsPage.notificationsSection.enableLabel')}</h3>
                                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('settingsPage.notificationsSection.enableDescription')}</p>
                                </div>
                                <label className="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" checked={isSubscribed} onChange={handleSubscriptionToggle} className="sr-only peer" disabled={isSaving} />
                            ...
                            <div className={`bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700 space-y-4 transition-opacity ${!isSubscribed ? 'opacity-50' : ''}`}>
                                <h3 className="font-semibold text-gray-800 dark:text-night-text">{t('settingsPage.notificationsSection.preferencesTitle')}</h3>
                                <div>
                                    <label htmlFor="tone" className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('settingsPage.notificationsSection.toneLabel')}</label>
                                    <select
                                        id="tone"
                                        value={prefs.tone}
                                        disabled={!isSubscribed}
                                        onChange={(e) => handlePrefChange('tone', e.target.value as NotificationTone)}
                                    ...
                                    >
                                        {NOTIFICATION_TONES.map(tone => <option key={tone} value={tone}>{t(`notificationTones.${tone}`)}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label htmlFor="length" className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('settingsPage.notificationsSection.lengthLabel')}</label>
                                    <select
                                        id="length"
                                        value={prefs.length}
                                        disabled={!isSubscribed}
                                        onChange={(e) => handlePrefChange('length', e.target.value as NotificationLength)}
                                        className="mt-1 block w-full pl-3 pr-10 py-2 text-base bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-dawn-purple focus:border-dawn-purple sm:text-sm rounded-md disabled:cursor-not-allowed"
                                    >
                                        {NOTIFICATION_LENGTHS.map(length => <option key={length} value={length}>{t(`notificationLengths.${length}`)}</option>)}
                                    </select>
                                </div>
                            </div>
                        </div>
